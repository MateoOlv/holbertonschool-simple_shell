...
int exepath(char **token)
{
	...
	if (!path)
	{
		fprintf(stderr, "%s: not recognized as acommand, program or executable batch file\n", token[0]);
		return (0);
	}
	pid = fork();
	if (pid == -1)
...
int execom(char *string, char **array)
{
	...
	{
		fprintf(stderr, "%s: not recognized as a command, program or executable batch file\n", array[0]);
		val = 127;
	}
	return (val);
}
